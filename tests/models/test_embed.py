...
@pytest.mark.parametrize(
    ...
    [("symanto/sn-xlm-roberta-base-snli-mnli-anli-xnli", 768)],
)
...
@pytest.mark.parametrize(
    "query,model_name,top_k",
    [
        ("foo foo foo", "symanto/sn-xlm-roberta-base-snli-mnli-anli-xnli", 1),
        ("foo foo foo", "symanto/sn-xlm-roberta-base-snli-mnli-anli-xnli", 3),
    ],
)
def test_most_similar(query, model_name, top_k, text_file, embeddings_file):
    data_file, *_ = text_file
    sim = embed.most_similar(
        query, model_name, data=data_file, embeddings=embeddings_file, top_k=top_k
    )
