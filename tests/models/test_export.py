...
def test_onnx(model_name, tmp_path):
    ...
    sent_emb = SentenceEmbeddings(model_name=model_name,
                                  device="cpu")
    ...
    input_feed = {k: v.detach().cpu().numpy() for k, v in zip(sent_emb.input_names,
                                               dummy)}
    ort_out = ort.run(output_names=sent_emb.output_names,
                      input_feed=input_feed)

    np.testing.assert_allclose(model_out[0].numpy(),
                               ort_out[0],
                               atol=1e-4,
                               rtol=1e-2)
...
def test_triton(model_name, tmp_path):
    ...
    assert (export_dir / "config.pbtxt").exists()
    assert (export_dir / "1" / "model.onnx").exists()
...
def test_triton_python_backend(model_name, tmp_path):
    ...
    model.export_tokenizer(export_dir,
                           "tokenizer",
                           version=1)
...
def test_triton_ensemble(model_name, tmp_path):
    model = SentenceEmbeddings(model_name)
    export_dir = Path(tmp_path) / "triton"
    model.export_ensemble(export_dir,
                           "ensemble",
                           version=1)

    assert (export_dir / "ensemble" / "1").exists()
    assert (export_dir / "ensemble" / "config.pbtxt").exists()
